...
A1:  We see primers in the reverse complement because the reads are long enough to catch a piece of the primer in their ends. We can use a different function to trim them later.
...
A2: We trim the forward reads at the drops in quality profiles, so like the last few nucleotides. In our data we're cutting off at 240 bps, excising the last 10.This will increase the overall average quality of the sequence.
...
A3: The reverse read quality is much worse than the forward reads, but it shouldn't be an issue since DADA2 incorporates these quality levels into a later error calculation.

Q4: Where will we trim our reverse reads and why?

A4: We trim the reverse reads at ~150 base pairs since the quality starts to drastically drop at that point.
...
A5: Ns signify ambiguous/unknown base calls.We won't be using these sequences.The MaxN section of filterAndTrim deals with this by allowing users to set the upper limit of how many we permit per read (in this case none).
...
A6: On average we're retaining around 93% Of the reads that pass through the filter.
...
A7: As the quality score of a sequence read increases, the error rate tends to decrease with a pretty linear relation.
...
A9: Nmatch tells us the number of matches in the overlap region of the forward and reverse region, which should be the same as we uniformly trimmed our data.
